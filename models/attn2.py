...
class EEG2ECGModel(pl.LightningModule):
    def __init__(
        ...
        self.eeg_min_frequency, self.eeg_max_frequency = 5, 30
        ...
        self.ecg_spectrogram_kernel_size = floor(self.ecg_sampling_rate / 8)
        ...
        if norm_fn == "instance":
            ...
            self.norm_fn = None
        self.eegs_latent_frequencies = ceil(
            self.eeg_spectrogram_frequencies / 2**self.layers_for_eegs
        )
        self.eegs_latent_samples = ceil(
            self.eeg_spectrogram_samples / 2**self.layers_for_eegs
        )
        self.eegs_encoder = self.build_encoder(
            ...
            dropout=0.0,
            latent_frequencies=self.eegs_latent_frequencies,
            latent_samples=self.eegs_latent_samples,
        )

        self.ecgs_latent_frequencies = ceil(
            self.ecg_spectrogram_frequencies / 2**self.layers_for_ecgs
        )
        self.ecgs_latent_samples = ceil(
            self.ecg_spectrogram_samples / 2**self.layers_for_ecgs
        )
        self.ecgs_encoder = self.build_encoder(
            ...
            dropout=dropout,
            architecture=self.architecture,
            latent_frequencies=self.ecgs_latent_frequencies,
            latent_samples=self.ecgs_latent_samples,
        )
        self.reasoner = self.build_classifier(
            in_features=self.h_dim, out_features=self.h_dim, use_batchnorm=True, dropout=0.,
        )

        self.eegs_decoder = self.build_decoder(
            out_channels=self.eeg_channels,
            out_frequencies=self.eeg_spectrogram_frequencies
            + self.padding_eeg[2]
            + self.padding_eeg[3],
            out_length=self.eeg_spectrogram_samples
            + self.padding_eeg[0]
            + self.padding_eeg[1],
            h_dim=self.h_dim,
            padding=self.padding_eeg,
    ...
    def configure_optimizers(self):
        ...
        sch = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
            opt,
            T_0=10,
            eta_min=self.learning_rate_eegs
            * (1e-1 if self.architecture == "conv" else 1e-2),
            verbose=False,
        )
        return [opt], [
        ...
        if architecture == "full":
            ...
            modules = {
                ...
                "post": nn.Sequential(
                    ...
                    nn.Linear(in_features=h_dim * latent_samples * latent_frequencies, out_features=h_dim),
                ),
            }
        if add_reasoner:
        ...
        if architecture == "full":
            ...
            modules = {
                "pre": nn.Sequential(
                    ...
                    #     padding=0,
                    # ),
                    nn.Linear(in_features=h_dim, out_features=h_dim * latent_samples * latent_frequencies),
                    Rearrange("b (c h w) -> b c h w", c=h_dim, h=latent_frequencies, w=latent_samples),
    ...
    def shared_step(self, batch, batch_idx):
        ...
        eegs_mel_spectrogram_gt = self.waveform_to_mel_spectrogram(
            waveform=eegs,
        ...
        ##################################
        ##################################
        with profiler.record_function("ecgs ae".upper()):
            ecg_latent = self.encode_mel_spectrogram(
                mel_spectrogram=ecgs_mel_spectrogram_gt,
                encoder=self.ecgs_encoder,
                pooled_outputs=True,
            ...
            outs["loss_G_ecg"] = self.reconstruction_loss(
                ecgs_mel_spectrogram_rec, ecgs_mel_spectrogram_gt, norm=1
            )
        ...
        ##################################
        with profiler.record_function("ecgs ae".upper()):
            eeg_latent = self.encode_mel_spectrogram(
            ...
            eegs_mel_spectrogram_rec = self.decode_mel_spectrogram(
                latent=eeg_latent,
                decoder=self.eegs_decoder,
                out_length=self.num_eeg_patches,
                expected_shape=(
                    batch_size,
                    self.eeg_channels,
                    self.eeg_spectrogram_frequencies,
            ...
            outs["loss_G_eeg"] = self.reconstruction_loss(
                eegs_mel_spectrogram_rec, eegs_mel_spectrogram_gt, norm=1
            )
        ...
        ##################################
        ##################################
        ecg_latent_gen = self.reasoner.classifier(eeg_latent)
        ...
        #     ),
        # )
        # outs["loss_G_ecg_gen"] = self.reconstruction_loss(
        #     ecgs_mel_spectrogram_gen, ecgs_mel_spectrogram_gt, norm=1
        # )

        outs["loss"] = outs["loss_G_ecg"] + outs["loss_G_eeg"] + outs["loss_R"]
        ...
        if (
            ...
            outs["images/rec_eeg_mel"] = wandb.Image(
                plot_reconstructed_spectrograms(
                    sg_pred=eegs_mel_spectrogram_rec[0, :4],
                    sg_gt=eegs_mel_spectrogram_gt[0, :4],
                    vmin=0,
                    # path=f"images/epoch={self.current_epoch}.png",
                ),
            ...
            outs["images/latent"] = wandb.Image(
                plot_reconstructed_spectrograms(
                    sg_pred=ecg_latent_gen.unsqueeze(1)[:4],
                    sg_gt=ecg_latent.unsqueeze(1)[:4],
                ),
    ...
    def encode_mel_spectrogram(
        ...
        mel_spectrogram = torch.log1p(mel_spectrogram + 1e-9)
        if self.architecture == "full":
            ...
            latent_tokens_transformed = encoder.post(x)
        if use_reasoner:
