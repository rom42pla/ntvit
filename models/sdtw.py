# code from https://github.com/Sleepwalking/pytorch-softdtw
import numpy as np
import torch
...
if __name__ == "__main__":
    ...
    y_gt = torch.randn(8, 128, 64, device="cuda")
    y_pred = torch.randn(8, 128, 64, device="cuda")
    st = time.time()
    loss_sdtw = criterion(y_pred, y_gt).mean()
    sdtw_time = time.time() - st
    
    st = time.time()
    loss_sdtw = F.mse_loss(y_pred, y_gt)
    mse_time = time.time() - st
    
    print(sdtw_time, mse_time)